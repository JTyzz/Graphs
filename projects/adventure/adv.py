...
player = Player("Tyzz", world.startingRoom)
...
visited_rooms = set()
traversal = {}
reverse_direction = {'n':'s', 'w':'e', 'e': 'w', 's':'n'}


def populate():
    #populate current unknown rooms with ?
    available_directions = {}

    if player.currentRoom.id not in traversal:
        for ext in player.currentRoom.getExits():
            available_directions[ext] = '?'
        traversal[player.currentRoom.id] = available_directions
...
def player_move(direction):
    ...
    populate()
    # set direction in both ways to correct paths
    traversal[prev_room][direction], traversal[new_room][reverse_direction[direction]] = new_room, prev_room
    # too many steps in big one bye 
    #print(traversal)
...
def unexplored_path():
    ...
    s = [[direction] for direction in traversal[player.currentRoom.id]]

    found = False

    while len(s) > 0 and found == False:
        path = s.pop()
        current_room = traversal[player.currentRoom.id][path[0]]
        for j in range(1, len(path)):
            current_room = traversal[current_room][path[j]]
        if current_room not in visited:
            visited.add(current_room)
            for next_room in traversal[current_room]:
                if traversal[current_room][next_room] == '?':
                    found = True
                    return [* path, next_room]
                s.insert(0, [*path, next_room])
...
while len(visited_rooms) < len(roomGraph):
    next_direction = None
    for open_direction in traversal[player.currentRoom.id]:
        if traversal[player.currentRoom.id][open_direction] == '?':
            next_direction = open_direction
            player_move(next_direction)
            break

    if next_direction == None:
        path = unexplored_path()
        if path == None:
            break
        for i in range(len(path) - 1):
            player.travel(path[i])
            traversalPath.append(path[i])
        player_move(path[-1])
